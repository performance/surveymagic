...
def analyze_single_question(question_column: str, df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # --- 3. Classification ---
    print(f"Classifying {len(participant_responses)} responses for {question_column}...")
    classifications = classify_responses(participant_responses, stable_themes)
    
    # Save classification details for inspection
    classification_df = pd.DataFrame(classifications.items(), columns=['ParticipantID', 'Theme'])
    classification_df['Response'] = classification_df['ParticipantID'].map(participant_responses)
    output_path = os.path.join(project_config.output_dir, f"{question_column}_classifications.xlsx")
    ...
    for theme in stable_themes:
        ...
        pids_in_theme = [pid for pid, t in classifications.items() if t == theme['theme_title']]
        responses_in_theme = [{'participant_id': pid, 'response': participant_responses[pid]} for pid in pids_in_theme]
        ...
        theme_data = {
            ...
            "participantCount": len(pids_in_theme),
            "participantPercentage": len(pids_in_theme) / total_participants_analyzed if total_participants_analyzed > 0 else 0,
    ...
    preliminary_analysis_data = {
        "questionText": canonical_question_text,
        "participants_analyzed": total_participants_analyzed,
        "themes": final_themes_with_quotes
    }
    narrative = generate_question_narrative(canonical_question_text, preliminary_analysis_data)
...
def main():
    ...
    # Use a ThreadPoolExecutor to analyze questions in parallel
    all_question_analyses = []
    with ThreadPoolExecutor() as executor:
        futures = {
            executor.submit(analyze_single_question, col, df): col
        ...
        for future in as_completed(futures):
            try:
                ...
                if result: # Only append if analysis was successful
                    all_question_analyses.append(result)
            except Exception as e:
                question_col = futures[future]
                print(f"An error occurred while processing {question_col}: {e}")
